// src/Chart.jsx
import { LabelList, LineChart, Line, BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, ReferenceLine, ComposedChart, Legend } from 'recharts';

export function MyComposeChart({title, data, valx, valy, unit, sep})
{
  const now = new Date().toLocaleString('fr-FR', {hour: '2-digit', timeZone: 'Europe/Paris'})
  return (
    <div style={styles.chart}>
      ...
      <ResponsiveContainer height={250}>
        <ComposedChart data={data}>
          ...
          <YAxis/>
          <Bar type="monotone" dataKey={valy.one} stroke="#8884d8" strokeWidth={1} dot={false} />
          <Line type="monotone" dataKey={valy.two} stroke="#afa601ff" strokeWidth={1} dot={false} />
          {/* <ReferenceLine 
            x={now}
            stroke="green" 
            strokeDasharray="3 3" 
            label={{ value: 'Demain', position: 'bottom', fill: 'red' }} 
          /> */}
          {sep ? <ReferenceLine 
            x={now}
            stroke="green" 
            strokeDasharray="3 3" 
            label={{ value: 'Now', position: 'insideTop', fill: 'green' }} 
          /> : (<></>)}
        </ComposedChart>
      </ResponsiveContainer>
    </div>
      ...
      <ResponsiveContainer height={250}>
        <BarChart data={data} margin={{ top: 0, right: 0, left: -25, bottom: 25 }}>
          ...
          {sep ? <ReferenceLine 
            ...
            stroke="red" 
            strokeDasharray="3 3" 
            label={{ value: 'Minuit', position: 'insideTop', fill: 'red' }} 
          /> : (<></>)}
        </BarChart>
      </ResponsiveContainer>
    </div>
    ...
    <div style={styles.chart}>
      ...
      <ResponsiveContainer height={250}>
        <LineChart data = {data} margin={{ top: 0, right: 0, left: -25, bottom: 25 }}>
          <XAxis dataKey={valx} interval={35} tick={{ fontSize: "15px" }} />
          <YAxis dataKey={valy} tick={{ fontSize: "10px" }}/>{/* 3. La bulle qui apparaît au survol */}
          <Tooltip />{/* 4. La ligne de données */}
